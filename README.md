...
```js
{
  ...
  date: "2026-07-30",
  cat: "패치",                // "패치" | "안내" | "공지". 목록 열에는 안 나오고
                             // 위쪽 분류 필터와 글 상세의 뱃지에만 쓰입니다
  pinned: false,             // true면 목록 맨 위 고정, 번호 대신 "공지" 표시
  title: "제목",              // 단문 패치노트. 아래 §제목 형식
  summary: "글 상세에서 제목 아래 한 줄로 보입니다",
...
제목은 **단문 패치노트**입니다. 바뀐 것을 쉼표로 나열하고 `업데이트` / `안내` / `공지`로 끝냅니다.

```
트레이너 NPC, 환상의 포켓몬 업데이트
체육관, 관장 스폰 업데이트
버그 해결 안내
```

- **날짜를 붙이지 않습니다.** 목록에 `날짜` 열이 따로 있어서 두 번 나옵니다.
- "체육관이 생겼습니다" 같은 서술형 문장 대신 무엇이 바뀌었는지 명사로 끊습니다.
- 느낌과 설명은 요약(`summary`)에서 풉니다. 제목은 훑어보면 바로 잡히게 짧게 둡니다.

목록은 **번호 · 제목 · 날짜** 세 열입니다. 분류와 요약은 목록에 나오지 않습니다
(분류는 위쪽 필터와 글 상세 뱃지, 요약은 글 상세에서 씁니다. 둘 다 검색에는 걸립니다).
